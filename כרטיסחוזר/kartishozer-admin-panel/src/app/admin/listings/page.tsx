...
import { getAdminListings } from "@/lib/queries/admin.queries";
import {
  AdminPageHeader, AdminFilterTabs, AdminBadge,
} from "@/components/admin/AdminComponents";
import { ListingReviewButtons } from "@/components/admin/ListingReviewButtons";
...
export default async function AdminListingsPage({ searchParams }: Props) {
  ...
  const RISK_FLAGS: { key: keyof typeof items[0]["riskAssessment"]; label: string }[] = [
            ...
            <tbody className="divide-y divide-white/5">
              {items.map((listing) => {
                const flags = listing.riskAssessment
                  ? RISK_FLAGS.filter((f) => (listing.riskAssessment as any)[f.key])
                  : [];
                ...
                return (
                  <tr key={listing.id} className="hover:bg-white/3 transition-colors">
                    ...
                    <td className="py-3 px-4">
                      <p className="font-bold text-white/80 truncate max-w-[160px]">
                        {listing.event.nameHe}
                      </p>
                      <p className="text-white/30 mt-0.5">
                        {fmtDate(listing.event.startsAt)} · {listing.event.city}
                      </p>
                      <p className="text-white/20 mt-0.5 font-mono text-[10px]">
                        {listing.id.slice(-8)}
                      </p>
                    </td>
                    ...
                    <td className="py-3 px-4">
                      <div className="flex items-center gap-1.5">
                        ...
                        <span className="font-black text-white/50 text-[11px]">
                          {listing.riskScore}
                        </span>
                      </div>
                    ...
                    <td className="py-3 px-4 max-w-[140px]">
                      <div className="flex flex-wrap gap-1">
                        ...
                        {flags.length === 0 && (
                          <span className="text-white/20 text-[10px]">ללא</span>
                        )}
                      </div>
                    </td>
