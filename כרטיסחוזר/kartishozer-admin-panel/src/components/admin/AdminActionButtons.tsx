...
import type { PlatformConfigEntry } from "@/lib/types/admin";
...
export function ListingReviewButtons({
  ...
  if (done) return (
    <span className="text-[11px] font-bold text-[#00B4A6]">{done} ✓</span>
  );
...
export function DisputeResolvePanel({ disputeId }: { disputeId: string }) {
  ...
  if (done) return (
    <span className="text-xs font-bold text-[#00B4A6]">הסכסוך נפתר ✓</span>
  );
