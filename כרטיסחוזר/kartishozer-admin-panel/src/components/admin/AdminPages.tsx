...
// Each in its own exported component — split into separate
// files when the codebase grows.
// ============================================================
...
import {
  getAdminDisputes, getAdminPayouts,
  getAdminVendors, getPlatformConfig, getAuditLog,
} from "@/lib/queries/admin.queries";
import {
  AdminPageHeader, AdminFilterTabs, AdminBadge,
} from "@/components/admin/AdminComponents";
import {
  DisputeResolvePanel, PayoutActionButtons,
              ...
              <div className="flex-1 space-y-1">
                <div className="flex items-center gap-2 flex-wrap">
                  ...
                  <span className="text-xs text-white/30">
                    {fmtDt(dispute.createdAt)}
                  </span>
                </div>
                <p className="text-sm font-black text-white/80">
                  {dispute.event.nameHe}
                </p>
                ...
                <p className="text-xs text-white/50 mt-2 rounded bg-white/5 px-3 py-2">
                  {dispute.reason}
                </p>
              </div>

              {/* Resolve panel — only for open disputes */}
          ...
          <thead>
            <tr className="border-b border-white/10 bg-white/5">
              {["מוכר", "אירוע", "סכום", "טריגר", "מתוזמן", "סטטוס", "פעולות"].map((h) => (
                <th key={h} className="py-3 px-4 font-bold text-white/30 uppercase tracking-wide">{h}</th>
              ))}
            </tr>
          </thead>
          <tbody className="divide-y divide-white/5">
            {items.map((payout) => (
              <tr key={payout.id} className="hover:bg-white/3">
                ...
                <td className="py-3 px-4 text-white/50 max-w-[120px] truncate">
                  {payout.order.event.nameHe}
                </td>
                <td className="py-3 px-4 font-black text-white/80">
                  {fmt(payout.amountAgorot)}
                </td>
                <td className="py-3 px-4 text-white/40">{payout.trigger}</td>
                <td className="py-3 px-4 text-white/40">{fmtDt(payout.scheduledFor)}</td>
                <td className="py-3 px-4"><AdminBadge value={payout.status} /></td>
                <td className="py-3 px-4">
            ...
            {items.map((vendor) => (
              <tr key={vendor.id} className="hover:bg-white/3">
                ...
                <td className="py-3 px-4 text-white/30">
                  {fmtDt(vendor.createdAt).split(",")[0]}
                </td>
                <td className="py-3 px-4"><AdminBadge value={vendor.status} /></td>
                <td className="py-3 px-4">
                  <VendorActionButtons
                    vendorId={vendor.id}
    ...
    <div>
      ...
      <div className="rounded-xl border border-white/10 overflow-hidden divide-y divide-white/5">
        {config.map((entry) => (
          <div
            key={entry.key}
            className="flex items-start justify-between gap-6 px-5 py-4 hover:bg-white/3"
          >
      ...
      <div className="mt-4 rounded-xl border border-[#E8503A]/20 bg-[#E8503A]/5 p-4">
        <p className="text-xs font-bold text-[#E8503A] mb-1">
          ⚠️ שינויים בהגדרות פועלים מיידית
        </p>
        <p className="text-xs text-white/40">
          שינוי עמלות ישפיע על כל ליסטינגים חדשים — לא ישפיע על עסקאות קיימות.
          שינוי סף ריסק ישפיע על ליסטינגים חדשים שעוברים בדיקת סיכון.
        </p>
      </div>
    </div>
  );
          ...
          <tbody className="divide-y divide-white/5">
            {items.map((log) => (
              <tr key={log.id} className="hover:bg-white/3">
                <td className="py-2.5 px-4 text-white/30 font-mono text-[11px]">
                  {fmtDt(log.createdAt)}
                </td>
                ...
                <td className="py-2.5 px-4 text-white/25 font-mono">
                  {log.resourceId.slice(-10)}
                </td>
