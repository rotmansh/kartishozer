...
export async function getPlatformStats(): Promise<PlatformStats> {
  ...
  const [
    ...
    // Platform revenue (ledger) last 30d
  ...
  const [rows, total] = await Promise.all([
    db.listing.findMany({
      ...
      select: {
        ...
        createdAt:          true,
        event:  { select: { nameHe: true, startsAt: true, city: true } },
...
export async function getAdminDisputes(options: {
  ...
  // Fetch requester emails from Clerk in production
  return {
    items: rows.map((r) => ({
      id:         r.id,
      type:       r.type,
      ...
      requestedBy: {
        id:    r.requestedById,
        email: "—", // hydrate from Clerk in production
      },
    })),
    total,
  };
...
export async function getAdminPayouts(options: {
  ...
  return { items: rows as AdminPayout[], total };
}
...
export async function getAdminVendors(options: {
  ...
  // Aggregate revenue + disputes per vendor
  const vendorIds = rows.map((r) => r.id);
  const [revenueAgg, disputeCounts] = await Promise.all([
    db.ledgerEntry.groupBy({
      by:    ["order"],
      where: {
        type:          "SELLER_PROCEEDS",
        creditAccount: "SELLER_PENDING",
        order:         { vendorId: { in: vendorIds } },
      },
      _sum: { amountAgorot: true },
    }),
    db.dispute.groupBy({
  ...
  return {
    items: rows.map((r) => ({
      ...
      totalEarnedAgorot: 0, // simplified — compute from ledger in production
      disputeCount:      0,
    })),
    total,
  };
